<footer id="footer">
	<div class="footer-content">
		<p>Copyright &copy; <?php echo date('Y'); ?> <a title="<?php bloginfo('name') ?>" href="<?php bloginfo('siteurl')?>"><?php bloginfo('name') ?></a> · 
		Powered by <a title="WordPress" href="https://cn.wordpress.org" target="_blank" rel="noopener">WordPress</a> · 
		Theme <a title="YLW3.0主题" href="https://www.yalewoo.com/ylw3.html" target="_blank" rel="noopener">YLW3</a></p>
		
		<div id="footer_menu">
			<?php wp_nav_menu(array('theme_location' => 'footer_menu')); ?>
		</div>
	</div>
	
	<script>
	var _hmt = _hmt || [];
	(function() {
		var hm = document.createElement("script");
		hm.src = "https://hm.baidu.com/hm.js?32ff821f324bdf2e2fc17726c3591cab";
		var s = document.getElementsByTagName("script")[0]; 
		s.parentNode.insertBefore(hm, s);
	})();
	</script>
</footer>

<div id="backtoTop">
	<span class="per" data-percent="0"></span>
</div>

<script type="text/javascript" src="<?php bloginfo('template_directory'); ?>/js/backtop.js"></script>
<?php wp_footer(); ?>

</body>
</html>